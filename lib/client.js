  ...
  factory: (require) => {
    ...
    (function () {
      var css = [
        ...
        ".dshm-check { display: inline-flex; align-items: center; gap: 4px; cursor: pointer; }",
        ".dshm-check input { accent-color: var(--dsw-alias-brand-primary); }",
        ".dshm-check input:disabled + span { opacity: .45; }",
    ...
    function MarketPage(props) {
      ...
      var stateCheckedSet = React.useState({});
      var checkedSet = stateCheckedSet[0], setCheckedSet = stateCheckedSet[1];
      ...
      // 集合多选：切换某子包是否勾选（已安装不可选）
      function spKeyFor(sp) { return selected.fullName + "@" + (sp.pkgPath || sp.dir); }
      function toggleCheck(key) {
        setCheckedSet(function (prev) {
      ...
      // 集合多选批量安装：按勾选顺序逐个调用 install/run（源码/或 npm），
      // 复用同仓库共享 clone 目录（首次 clone，后续原地更新），一次勾选全部装完。
      function batchInstall(subs) {
        var sel = (subs || []).filter(function (sp) { return !!(checkedSet && checkedSet[spKeyFor(sp)]) && !sp.installed; });
        if (!sel.length) return;
        var results = { ok: [], skipped: [], failed: [] };
        setInstall({ status: "running", installingBundle: true, batchTotal: sel.length, batchIndex: 0, batchResults: null });
        function next(i) {
          if (i >= sel.length) {
            setCheckedSet({});
            setInstall({ status: "done", payload: { batch: true }, bundleInstalled: true, batchTotal: sel.length, batchResults: results });
            markDetailInstalled();
            return;
          }
          var sp = sel[i];
          setInstall({ status: "running", installingBundle: true, batchTotal: sel.length, batchIndex: i, batchResults: null });
          if (sp.npm && sp.npm.available) {
            callApi("install/run", { kind: "npm", pkgName: sp.name }).then(function (r) {
              if (r && r.ok) { results.ok.push(r.pkgName || sp.name || sp.dir); } else { results.failed.push({ name: r && r.pkgName || sp.name || sp.dir, error: (r && r.error) || "npm 安装失败" }); }
              next(i + 1);
            }, function (e) { results.failed.push({ name: sp.name || sp.dir, error: String((e && e.message) || e) }); next(i + 1); });
          } else if (sp.installable) {
            callApi("install/run", { fullName: selected.fullName, kind: "bundle", pkgPath: sp.pkgPath }).then(function (r) {
              if (r && r.ok) { results.ok.push(r.pkgName || sp.name || sp.dir); } else { results.failed.push({ name: r && r.pkgName || sp.name || sp.dir, error: (r && r.error) || "源码安装失败" }); }
              next(i + 1);
            }, function (e) { results.failed.push({ name: sp.name || sp.dir, error: String((e && e.message) || e) }); next(i + 1); });
          } else {
            results.skipped.push(sp.name || sp.dir);
            next(i + 1);
          }
        }
        next(0);
      ...
      function renderDetail() {
        ...
        rows.push(React.createElement("button", { key: "install", className: "dshm-btn dshm-btn-primary", disabled: !!(install && (install.status === "running" || install.status === "pick")), onClick: doInstall },
          install && install.status === "running" ? (install.installingBundle ? (install.batchTotal ? "正在批量安装 " + (install.batchIndex + 1) + "/" + install.batchTotal + "…" : "正在安装（克隆源码）…") : "获取源码中…") : (detail.installed ? "已安装（重新获取源码）" : "一键安装")));
        ...
        if (install && install.status === "pick") {
          ...
          if (!subs.length && pl.rootNpm && pl.rootNpm.available) {
            ...
            var checkedCount = subs.filter(function (sp) { return !!(checkedSet && checkedSet[spKeyFor(sp)]) && !sp.installed; }).length;
            var inner = [React.createElement("div", { key: "t", className: "dshm-name" }, "该仓库是插件集合（" + subs.length + " 个可选，已按类分组）：")];
            inner.push(React.createElement("div", { key: "batchbar", className: "dshm-manage-actions" },
              React.createElement("button", { className: "dshm-btn dshm-btn-primary", disabled: !checkedCount, title: checkedCount ? "批量安装所选的 " + checkedCount + " 个子包" : "勾选要安装的子包后再批量安装", onClick: function () { batchInstall(subs); } }, "批量安装所选（" + checkedCount + "）")
            ));
            groups.forEach(function (g) {
              ...
              g.items.forEach(function (sp) {
                var installed = !!sp.installed;
                inner.push(React.createElement("div", { key: g.key + sp.dir, className: "dshm-manage-row" },
                  React.createElement("label", { className: "dshm-check" },
                    React.createElement("input", { type: "checkbox", checked: !!(checkedSet && checkedSet[spKeyFor(sp)]), disabled: installed, onChange: function () { toggleCheck(spKeyFor(sp)); } }),
                    React.createElement("span", null)
                  ),
                  React.createElement("span", { className: "dshm-name" }, sp.name || sp.dir),
                  installed ? React.createElement("span", { className: "dshm-badge dshm-badge-installed" }, "已安装") : null,
                  React.createElement("div", { className: "dshm-manage-actions" },
                    sp.npm && sp.npm.available
                      ? React.createElement("button", { className: "dshm-btn dshm-btn-primary", disabled: installed, title: installed ? "已安装，如需重装请先卸载" : "npm 安装（官方推荐，自动处理依赖与构建）", onClick: function () { runNpmInstall(sp.name); } }, installed ? "已安装" : "npm 安装" + (sp.npm.version ? " v" + sp.npm.version : ""))
                      : null,
                    sp.installable
                      ? React.createElement("button", { className: "dshm-btn", disabled: installed, title: installed ? "已安装，如需重装请先卸载" : "从 GitHub 源码安装", onClick: function () { runBundleInstall(sp.pkgPath); } }, installed ? "已安装" : "源码安装")
                      : null
                  )
                ));
              });
        ...
        if (install && install.status === "done") {
          ...
          if (install.batchResults) {
            var br = install.batchResults;
            inner.push(React.createElement("div", { key: "b0", className: "dshm-ok" }, "✓ 批量安装完成：成功 " + br.ok.length + " 个" + (br.failed.length ? " · 失败 " + br.failed.length + " 个" : "") + (br.skipped.length ? " · 跳过 " + br.skipped.length + " 个" : "")));
            if (br.ok.length) inner.push(React.createElement("div", { key: "b0a", className: "dshm-desc" }, "已安装：" + br.ok.join("、")));
            if (br.failed.length) inner.push(React.createElement("div", { key: "b0b", className: "dshm-error" }, "失败：" + br.failed.map(function (f) { return (f.name || "") + (f.error ? "（" + f.error + "）" : ""); }).join("；")));
            if (br.skipped.length) inner.push(React.createElement("div", { key: "b0c", className: "dshm-desc" }, "跳过：" + br.skipped.join("、") + "（不可安装或已安装）"));
            inner.push(React.createElement("div", { key: "b0d", className: "dshm-desc" }, "重启 DSH 后生效。"));
          } else if (install.bundleInstalled) {
