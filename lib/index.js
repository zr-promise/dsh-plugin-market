...
export function apply(ctx) {
  ...
  async function runNpmUpdateRpc(name, signal) {
    ...
    try {
      ...
      return { ok: false, error: 'npm 更新失败：' + String((err && err.message) || err) }
    }
    if (!touched.length) {
      return { ok: false, error: '未找到可更新的 npm 安装：' + (lastError || name) }
    }
    installedScan = null
    return { ok: true, name, npm: true, profiles: touched, restartRequired: true, note: '已通过 npm 更新到最新版，重启 DSH 后生效' }
  }
  ...
  async function manageUpdateRpc(name, signal) {
    ...
    if (existsSync(join(repoRoot, '.git'))) {
      ...
      if (!plugin.repoFullName) return await runNpmUpdateRpc(name, signal)
      ...
      if (others.length) return { ok: false, error: '该目录是共享仓库（含其他已装插件）且本地无 .git，无法安全自动更新；请手动 git pull' }
      try {
    ...
    if (!existsSync(join(pkgDir, 'package.json'))) {
      if (!existsSync(join(repoRoot, '.git'))) { try { rmSync(repoRoot, { recursive: true, force: true }) } catch { /* ignore */ } }
      return { ok: false, error: '更新后未找到 package.json：' + pkgDir }
    }
    const pj = readJsonFile(join(pkgDir, 'package.json'))
    if (!pj) {
      if (!existsSync(join(repoRoot, '.git'))) { try { rmSync(repoRoot, { recursive: true, force: true }) } catch { /* ignore */ } }
      return { ok: false, error: '更新后 package.json 无法解析：' + pkgDir }
    }
    ...
    if (pkgDeps.length) {
      ...
      if (!r.ok) return { ok: false, error: '更新后安装依赖失败：' + r.error }
    }
    const vRes = await verifyBundleResolvable(pkgDir, pj)
    if (!vRes.ok) return { ok: false, error: '更新后校验失败：' + vRes.error }
    installedScan = null
