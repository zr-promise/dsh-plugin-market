 ...
 */
import { readFileSync, writeFileSync, rmSync, mkdirSync, readdirSync, realpathSync, existsSync, statSync, lstatSync, symlinkSync, cpSync } from 'node:fs'
...
const CURATED = [
  ...
  { fullName: 'yjh051108/dsh-routing-suite', category: '插件', note: '注入器 × 思维模式路由套装（P1-P23 实测）' },
]
...
export function apply(ctx) {
  ...
  // 已知“套装/预设”类安装：卸载对应 bundle 插件时，把关联的源码目录与 agent preset 一并清掉。
  // 目前覆盖 dsh-routing-suite（injector + router presets）；后续有新套装在此追加。
  const SUITE_EXTRAS = {
    '@dsh-external/dsh-super-injector': {
      label: 'dsh-routing-suite',
      components: () => [
        { path: join(dshHome(), 'injector-pkg'), label: 'injector 源码' },
        { path: join(dshHome(), '.agent-presets', 'router-standard'), label: 'router-standard 预设' },
        { path: join(dshHome(), '.agent-presets', 'router-spec'), label: 'router-spec 预设' },
        { path: join(dshHome(), '.agent-presets', 'router-pro'), label: 'router-pro 预设' },
      ],
    },
  }

  // 已知本地副本可能不带 repository/.git 的插件 → 直接补全仓库名，保证“检查更新”可用。
  const KNOWN_PLUGIN_REPOS = {
    '@dsh-external/dsh-plugin-market': 'zr-promise/dsh-plugin-market',
  ...
  // 通用“套装清单”约定：第三方套装安装器可在 ~/.dsh/dsh-suite-manifests/*.json
  // 写入描述文件，市场就能自动识别并支持整体卸载。
  function suiteManifestDir() { return join(dshHome(), 'dsh-suite-manifests') }
  function resolveSuitePath(p) {
    if (typeof p !== 'string' || !p.trim()) return null
    const s = p.trim().replace(/\\/g, '/')
    if (/^[a-zA-Z]:\//.test(s) || s.startsWith('/')) return p.trim()
    return join(dshHome(), p.trim())
  }
  function loadSuiteManifests() {
    const out = []
    let entries = []
    try { entries = readdirSync(suiteManifestDir(), { withFileTypes: true }) } catch { return out }
    for (const e of entries) {
      if (!e.isFile() || !/\.json$/i.test(e.name)) continue
      const data = readJsonFile(join(suiteManifestDir(), e.name))
      if (!data || typeof data.id !== 'string' || !Array.isArray(data.plugins)) continue
      const components = Array.isArray(data.components) ? data.components
        .filter((c) => c && typeof c.label === 'string' && typeof c.path === 'string')
        .map((c) => ({ label: c.label, path: resolveSuitePath(c.path) }))
        .filter((c) => c.path) : []
      const repos = data.repos && typeof data.repos === 'object' ? data.repos : {}
      out.push({ id: data.id, label: typeof data.label === 'string' && data.label ? data.label : data.id, plugins: data.plugins.filter((p) => typeof p === 'string'), repos, components })
    }
    return out
  }

  // 更新备份/回滚：更新前把旧源码（或 npm profile 元数据）复制到备份目录，失败时恢复。
  function backupRootDir() { return join(dshHome(), 'dsh-plugin-market-backups') }
  function makeBackupDir(name) {
    const dir = join(backupRootDir(), name.replace(/[^a-z0-9@/-]/gi, '_') + '-' + Date.now())
  ...
  function restoreTree(backup, target) {
  ...
  // 从 staging（GUI 已调用 install 时缓存）取出该仓库的子包列表。
  function getStagedSubPackages(fullName) {
    const staged = staging.get(fullName)
    return (staged && Array.isArray(staged.subPackages)) ? staged.subPackages : []
  }

  // 仓库 clone 目录是否正被某个已安装包的引用路径占用（插件集合的其它子包）。
  // 被占用（如 skin-manager 已装、dir 为 dsh-deep-whale/skin-manager）时不可删除该
  // 目录，否则会破坏已安装子包的 link。
  function repoDirReferenced(target) {
    const t = target.replace(/\\/g, '/').replace(/\/+$/, '')
    return scanInstalled().paths.some((x) => {
      const p = (x.path || '').replace(/\\/g, '/').replace(/\/+$/, '')
      return p === t || p.indexOf(t + '/') === 0
    })
  }
  ...
  function describeInstalledPlugin(name, path, profile, from) {
    ...
    // 本地元数据缺失时，用已知映射/套装清单补全仓库名，保证“检查更新”可用。
    if (!item.repoFullName && KNOWN_PLUGIN_REPOS[name]) item.repoFullName = KNOWN_PLUGIN_REPOS[name]
    if (!item.repoFullName) {
      for (const m of loadSuiteManifests()) {
        if (m.plugins.indexOf(name) >= 0 && typeof m.repos[name] === 'string') {
          item.repoFullName = m.repos[name]
          break
        }
      }
    }
    const builtinSuite = SUITE_EXTRAS[name]
    const manifestSuites = loadSuiteManifests().filter((m) => m.plugins.indexOf(name) >= 0)
    if (builtinSuite || manifestSuites.length) {
      const byPath = new Map()
      if (builtinSuite) {
        for (const c of builtinSuite.components()) {
          if (c.path && !byPath.has(c.path.replace(/\\/g, '/').toLowerCase())) byPath.set(c.path.replace(/\\/g, '/').toLowerCase(), c)
        }
      }
      for (const m of manifestSuites) {
        for (const c of m.components) {
          if (c.path && !byPath.has(c.path.replace(/\\/g, '/').toLowerCase())) byPath.set(c.path.replace(/\\/g, '/').toLowerCase(), c)
        }
      }
      const comps = [...byPath.values()].filter((c) => existsSync(c.path))
      item.suiteLabel = (manifestSuites[0] && manifestSuites[0].label) || (builtinSuite && builtinSuite.label) || name
      item.suiteComponents = comps.map((c) => c.label)
      item.extraCleanupDirs = comps.map((c) => c.path)
    }
    return item
  }
  ...
  // 简单 semver 比较：支持 x.y.z，忽略 prerelease/build 后缀；无法解析时按字符串相等判断。
  ...
  // 本地没有仓库信息时，回退到 npm registry 的 repository 元数据兜底。
  async function checkPluginUpdate(plugin, signal) {
    let repoFullName = plugin.repoFullName
    if (!repoFullName) {
      const npm = await npmCheck(plugin.name, signal)
      if (npm.available && npm.repo) repoFullName = npm.repo
    }
    if (!repoFullName) return Object.assign({}, plugin, { checkState: 'no-repo' })
    const key = 'update:' + repoFullName
    const now = Date.now()
    if (updateCache.has(key) && now - updateCache.get(key).at < UPDATE_TTL) {
      return Object.assign({}, plugin, { repoFullName }, updateCache.get(key).value)
    }
    const res = await ghGet('/repos/' + repoFullName, null, signal)
    if (!res.ok) {
      const value = { checkState: 'error', checkError: res.error || ('HTTP ' + res.status) }
      updateCache.set(key, { at: now, value })
      return Object.assign({}, plugin, { repoFullName }, value)
    }
    const r = res.json
    let remoteVersion = null
    let remotePath = null
    if (plugin.hasPackageJson && plugin.pkgRelPath) {
      const raw = await rawGet(repoFullName, r.default_branch || 'main', plugin.pkgRelPath, signal)
      if (raw) {
    ...
    // 优先按版本号判断更新；版本可比较时不再用“推送时间”误报。
    const cmp = compareVersions(remoteVersion, plugin.version)
    let hasUpdate = null
    let note = '无法比较版本与时间'
    if (cmp !== null) {
      hasUpdate = cmp > 0
      note = hasUpdate ? '远程版本高于本地版本，建议更新' : (cmp === 0 ? '远程与本地版本一致，已是最新' : '远程版本不高于本地版本，已是最新')
    } else if (Number.isFinite(remoteTs) && localTs > 0) {
      hasUpdate = remoteTs > localTs
      note = hasUpdate ? '无法比较版本，按仓库推送时间判断有新提交' : (hasUpdate === false ? '已是最新' : '无法比较时间')
    }
    const value = {
      checkState: 'ok',
    ...
    if (ref && ref.path) {
      ...
      try {
        ...
        for (const e of pEntries) {
          ...
          // 用 lstatSync 而非 existsSync：卸载时源码子目录已删除，junction 变成断链，
          // existsSync（跟随链接）会返回 false 而跳过清理，留下悬空 junction/链接。
          try { lstatSync(nm) } catch { continue }
          try { rmSync(nm, { recursive: true, force: true }) } catch { /* ignore */ }
          clearPnpmState(join(profilesDir, e.name))
        }
      } catch { /* ignore */ }
    }

    // ── 套装/预设清理：删除关联的源码目录与 agent preset（如 dsh-routing-suite）──
    const suiteCleanup = []
    const builtinSuite = SUITE_EXTRAS[name]
    if (builtinSuite) suiteCleanup.push(...builtinSuite.components())
    for (const m of loadSuiteManifests().filter((m) => m.plugins.indexOf(name) >= 0)) {
      suiteCleanup.push(...m.components)
    }
    const seenCleanup = new Set()
    for (const c of suiteCleanup) {
      const dir = c.path
      if (!dir || !existsSync(dir)) continue
      const norm = dir.replace(/\\/g, '/').toLowerCase()
      if (seenCleanup.has(norm)) continue
      seenCleanup.add(norm)
      const homeNorm = dshHome().replace(/\\/g, '/').toLowerCase() + '/'
      if (norm.indexOf(homeNorm) !== 0) continue
      try {
        rmSync(dir, { recursive: true, force: true })
        removedDirs.push(dir)
      } catch (err) {
        cleanNote += '（套装组件删除失败：' + String((err && err.message) || err).slice(0, 200) + '，可手动删除 ' + dir + '）'
      }
    }
    // 如果“包不在 plugin-src 下”的源码目录实际已被套装清理删除，就不再显示误导性的未删除提示。
    if (ref && ref.path && removedDirs.some((d) => d.replace(/\\/g, '/').toLowerCase() === ref.path.replace(/\\/g, '/').toLowerCase())) {
      cleanNote = ''
    }
    installedScan = null
    return {
      ...
      note: '已从 profile 移除' + (removedDirs.length ? '，并删除源码/套装组件：' + removedDirs.join('；') : '') + cleanNote + '。重启 DSH 后生效',
    }
  }

  // 更新：优先更新 git 源码（pull/reset 或重新 clone），npm 安装的走 pnpm add @latest。
  // 更新前自动备份，失败时回滚 package.json / lockfile / node_modules。
  async function runNpmUpdateRpc(name, signal) {
    const profilesDir = join(dshHome(), 'profiles')
    const jobs = []
    const touched = []
    let lastError = null
    const rollbackAll = async () => {
      for (const j of jobs) {
        try {
          const pkgBackup = join(j.backup, 'package.json')
          const lockBackup = join(j.backup, 'pnpm-lock.yaml')
          if (existsSync(pkgBackup)) cpSync(pkgBackup, join(j.profileDir, 'package.json'), { force: true })
          if (existsSync(lockBackup)) cpSync(lockBackup, join(j.profileDir, 'pnpm-lock.yaml'), { force: true })
          await pnpmInstall(j.profileDir, signal)
        } catch { /* ignore rollback error */ }
      }
      try { rmSync(backupRootDir(), { recursive: true, force: true }) } catch { /* ignore */ }
    }
    try {
      const entries = readdirSync(profilesDir, { withFileTypes: true })
      for (const e of entries) {
        if (!e.isDirectory()) continue
        const pj = readJsonFile(join(profilesDir, e.name, 'package.json'))
        if (!pj || !pj.dependencies || typeof pj.dependencies[name] !== 'string') continue
        const depVal = pj.dependencies[name]
        if (typeof depVal === 'string' && (depVal.indexOf('link:') === 0 || depVal.indexOf('file:') === 0)) continue
        const profileDir = join(profilesDir, e.name)
        const backup = makeBackupDir(name + '-npm')
        try {
          cpSync(join(profileDir, 'package.json'), join(backup, 'package.json'))
          const lockPath = join(profileDir, 'pnpm-lock.yaml')
          if (existsSync(lockPath)) cpSync(lockPath, join(backup, 'pnpm-lock.yaml'))
        } catch (err) {
          return { ok: false, error: '创建 npm 更新备份失败：' + String((err && err.message) || err) }
        }
        jobs.push({ profileDir, name: e.name, backup })
        const npmInfo = await npmCheck(name, signal)
        const installSpec = npmInfo.available && npmInfo.version ? (name + '@' + npmInfo.version) : (name + '@latest')
        const r = await pnpmInstall(profileDir, signal, [installSpec], 'add')
        if (!r.ok) {
          lastError = r.error
          await rollbackAll()
          return { ok: false, error: 'npm 更新失败：' + (lastError || '未知错误') + '（已回滚）' }
        }
        touched.push(e.name)
      }
    } catch (err) {
      await rollbackAll()
      return { ok: false, error: 'npm 更新失败：' + String((err && err.message) || err) + '（已回滚）' }
    }
    if (!touched.length) {
      try { rmSync(backupRootDir(), { recursive: true, force: true }) } catch { /* ignore */ }
      return { ok: false, error: '未找到可更新的 npm 安装：' + (lastError || name) }
    }
    try { rmSync(backupRootDir(), { recursive: true, force: true }) } catch { /* ignore */ }
    installedScan = null
    return { ok: true, name, npm: true, profiles: touched, restartRequired: true, note: '已通过 npm 更新到最新版（已自动清理备份），重启 DSH 后生效' }
  }

  async function manageUpdateRpc(name, signal) {
    if (typeof name !== 'string' || !name.trim()) return { ok: false, error: '缺少插件名' }
    name = name.trim()
    const all = listInstalledPlugins()
    const plugin = all.find((p) => p.name === name)
    if (!plugin) return { ok: false, error: '未找到插件：' + name }
    if (plugin.referenced === false) return { ok: false, error: '源码残留未安装，无法更新' }
    // npm 安装的包位于 node_modules 下，即使有 repoFullName 也应走 pnpm 更新，而不是替换成 git 源码。
    if ((plugin.path || '').replace(/\\/g, '/').indexOf('/node_modules/') >= 0) {
      return await runNpmUpdateRpc(name, signal)
    }
    ...
    let repoRoot = null
    let subDir = ''
    if (plugin.pkgRelPath && plugin.pkgRelPath.indexOf('/') >= 0) {
      subDir = plugin.pkgRelPath.split('/').slice(0, -1).join('/')
    }
    if (normPath.indexOf(normRoot) === 0) {
      const rel = normPath.slice(normRoot.length)
      const repoDir = rel.split('/')[0]
      repoRoot = join(srcRoot, repoDir)
    } else {
      repoRoot = plugin.path
    }
    if (!repoRoot || !existsSync(repoRoot)) return { ok: false, error: '本地安装路径不存在：' + (repoRoot || plugin.path) }

    // 更新前备份旧源码（跳过 node_modules，回滚后依赖会重新安装）
    const backup = makeBackupDir(name)
    try {
      copyTree(repoRoot, backup)
    } catch (err) {
      return { ok: false, error: '创建更新备份失败：' + String((err && err.message) || err) }
    }
    const rollback = () => {
      try { restoreTree(backup, repoRoot) } catch { /* ignore */ }
    }
    const cleanupBackup = () => {
    ...
    if (existsSync(join(repoRoot, '.git'))) {
      updateRes = await gitPull(repoRoot, signal)
    } else {
      // 无 .git 时：没有仓库信息则走 npm 更新；否则重新 clone。
      if (!plugin.repoFullName) {
        cleanupBackup()
        return await runNpmUpdateRpc(name, signal)
      }
      // 共享仓库（含其他已装插件）不能整目录删除，需走 git pull。
      const others = all.filter((p) => p.name !== name && p.path && p.path.replace(/\\/g, '/').toLowerCase().indexOf(repoRoot.replace(/\\/g, '/').toLowerCase() + '/') === 0)
      if (others.length) {
        cleanupBackup()
        return { ok: false, error: '该目录是共享仓库（含其他已装插件）且本地无 .git，无法安全自动更新；请手动 git pull' }
      }
      try {
        rmSync(repoRoot, { recursive: true, force: true })
      } catch (err) {
        cleanupBackup()
        return { ok: false, error: '无法清理旧目录：' + String((err && err.message) || err) }
      }
      updateRes = await cloneGit('https://github.com/' + plugin.repoFullName + '.git', repoRoot, signal)
    }
    if (!updateRes.ok) {
      rollback()
      cleanupBackup()
      return { ok: false, error: (updateRes.error || '更新失败') + '（已回滚）' }
    }

    const pkgDir = subDir ? join(repoRoot, subDir) : repoRoot
    if (!existsSync(join(pkgDir, 'package.json'))) {
      rollback()
      cleanupBackup()
      return { ok: false, error: '更新后未找到 package.json：' + pkgDir + '（已回滚）' }
    }
    const pj = readJsonFile(join(pkgDir, 'package.json'))
    if (!pj) {
      rollback()
      cleanupBackup()
      return { ok: false, error: '更新后 package.json 无法解析：' + pkgDir + '（已回滚）' }
    }
    const pkgDeps = (pj.dependencies && typeof pj.dependencies === 'object') ? Object.keys(pj.dependencies) : []
    if (pkgDeps.length) {
      const r = await pnpmInstall(pkgDir, signal, ['--prod', '--ignore-scripts'])
      if (!r.ok) {
        rollback()
        cleanupBackup()
        return { ok: false, error: '更新后安装依赖失败：' + r.error + '（已回滚）' }
      }
    }
    const vRes = await verifyBundleResolvable(pkgDir, pj)
    if (!vRes.ok) {
      rollback()
      cleanupBackup()
      return { ok: false, error: '更新后校验失败：' + vRes.error + '（已回滚）' }
    }
    cleanupBackup()
    installedScan = null
    return { ok: true, name, path: pkgDir, restartRequired: true, note: '已更新到最新源码（' + (plugin.repoFullName || repoRoot) + '），重启 DSH 后生效' }
  }
  ...
  function parseCurlOutput(text) {
    // -L 跟随重定向时 -D - 输出多个头块（301 + 200…）：取最后一个状态行，
    // 并从该状态行向后找第一个空行作为头/体分隔。不能用全局 lastIndexOf('\r\n\r\n')：
    // 若响应体本身含 CRLF 空行（如 Windows 行尾的 README），会把正文截断。
    const all = [...text.matchAll(/^HTTP\/\S+ (\d{3})/gm)]
    let headers = text
    let body = ''
    if (all.length) {
      ...
      const sep = /\r?\n\r?\n/.exec(rest)
      if (sep) {
        const sepEnd = from + sep.index + sep[0].length
        headers = text.slice(0, sepEnd)
        body = text.slice(sepEnd)
      }
    }
  ...
  // 仅用于 `dsh in:name` 补充源：名字里的 "dsh" 必须是独立词（dsh、dsh-、dsh_、dsh. 等），
  // 避免 Dshell / DSharpPlus / DsHidMini 这类恰好包含 "dsh" 子串的非 DSH 项目混入首页。
  function isDshName(name) {
    return /(^|[^a-z0-9])dsh([^a-z0-9]|$)/i.test(name || '')
  }
  ...
  async function searchRepos(query, refresh, page, signal) {
    ...
    let quota = null
    if (!q) {
      // 首页浏览：topic:dsh-plugin 会漏掉“名字/描述含 dsh 但没打 topic”的高星仓库，
      // 所以再并一路 dsh in:name（按 stars 排序），合并去重后展示。
      const queries = ['topic:dsh-plugin', 'dsh in:name']
      let firstError = null
      for (const query of queries) {
        const res = await ghGet('/search/repositories', Object.assign({ q: query }, base), signal)
        if (!res.ok) {
          if (!raw.length) firstError = res
          continue
        }
        const items = (res.json.items || []).map(githubItem)
        raw = mergeItems(raw, query === 'dsh in:name' ? items.filter((i) => isDshName(i.name)) : items)
        totalCount += typeof res.json.total_count === 'number' ? res.json.total_count : 0
        quota = res.quota
      }
      if (!raw.length) {
        return { items: withInstalled(curatedMatches), live: false, error: 'GitHub 搜索失败：' + ((firstError && (firstError.error || (firstError.json && firstError.json.message) || ('HTTP ' + firstError.status))) || '无结果') + '（' + (authToken ? '认证限流：搜索 30 次/分钟' : '未认证限流：搜索 10 次/分钟、API 60 次/小时') + '）', auth: authState, login: authLogin, quota: null }
      }
    } else {
      ...
      if (raw.length < 5 && pg === 1) {
        const broad = await ghGet('/search/repositories', Object.assign({ q: q + ' in:name,description,readme', per_page: 100 }, base), signal)
        if (broad.ok) {
  ...
  async function collectManifestInner(fullName, signal) {
    ...
    // 标注子包安装状态（插件集合语义：按子包包名判定，而非仓库级别）。
    for (const sp of subPackages) {
      sp.installed = !!(sp && sp.name && scanInstalled().paths.some((x) => x.name === sp.name))
    }
  ...
  async function installSource(fullName, signal) {
    const staged = staging.get(fullName)
    if (staged) {
      // 重新标注子包安装状态（staging 缓存的是安装前快照，装完会变 stale）
      const subs = (staged.subPackages || []).map((sp) => {
        if (!sp || !sp.name) return sp
        const s = Object.assign({}, sp)
        s.installed = scanInstalled().paths.some((x) => x.name === s.name)
        return s
      })
      return { ok: true, staged: true, fullName: staged.fullName, branch: staged.branch, files: staged.files, install: staged.install, pkgPath: staged.pkgPath, subPackages: subs, rootNpm: staged.rootNpm, rootInstallable: staged.rootInstallable, treeSummary: staged.treeSummary, totalFiles: staged.totalFiles, hint: staged.hint, addCmd: staged.addCmd, installed: isInstalled({ fullName }) }
    }
    ...
    if (res.install && res.install.kind === 'dynamic') {
      hint += ' 安装流程：对助手说“安装 ' + fullName + '”，助手会调用 market_install 读取源码，再执行 cordis_define 定义 Package、cordis_run 激活（Client 半侧首次运行需在 GUI 中批准）。'
    } else if (res.install && res.install.kind === 'bundle') {
      if (res.subPackages && res.subPackages.length > 1) {
        hint += ' 该仓库是插件集合，含 ' + res.subPackages.length + ' 个子包，请在 GUI 中选择要安装的子包；选择后由 Host 自动完成 clone + 写 profile + 依赖同步。'
      } else if (addCmd) {
        hint += ' 在 GUI 点击“一键安装”即可由 Host 自动完成 clone + 写 profile + 依赖同步；也可复制命令：`' + addCmd + '`。'
      }
    } else {
      hint += ' 该仓库不可直接安装，仅供信息参考。'
    }
    const payload = {
  ...
  // 更新已存在的 git 源码目录：fetch + reset 到远端默认分支（保留 .git，适合共享仓库）。
  async function gitPull(dir, signal) {
    let exe
    try { exe = await subprocess.resolveExecutable('git') } catch (err) {
      return { ok: false, error: '未找到 git：' + String((err && err.message) || err) }
    }
    const run = async (args) => {
      ...
      const errText = ((errOut && errOut.text) || '').slice(-500)
      if (outcome.exitCode !== 0) {
        return { ok: false, error: 'git ' + args.join(' ') + ' 失败（exit ' + outcome.exitCode + '）：' + (errText || '(无错误输出)') }
      }
      return { ok: true, out: ((out && out.text) || '').slice(-500) }
    }
    const branchRes = await run(['-C', dir, 'rev-parse', '--abbrev-ref', 'HEAD'])
    if (!branchRes.ok) return branchRes
    const branch = branchRes.out.trim()
    const fetchRes = await run(['-C', dir, 'fetch', 'origin'])
    if (!fetchRes.ok) return fetchRes
    if (!branch) return { ok: false, error: '无法确定当前分支' }
    return await run(['-C', dir, 'reset', '--hard', 'origin/' + branch])
  }
  ...
  async function runNpmInstallRpc(pkgName) {
    ...
    // 显式安装 registry 最新版本，避免 pnpm v11 的 minimumReleaseAge 静默装到旧版。
    const npmInfo = await npmCheck(pkgName, undefined)
    const installSpec = npmInfo.available && npmInfo.version ? (pkgName + '@' + npmInfo.version) : pkgName
    const profilesDir = join(dshHome(), 'profiles')
    const touched = []
    let lastError = null
    try {
      ...
      for (const e of entries) {
        if (!e.isDirectory()) continue
        const profileDir = join(profilesDir, e.name)
        let r = await pnpmInstall(profileDir, undefined, [installSpec], 'add')
        if (r.ok && !profileLinkOk(profileDir, pkgName)) {
          clearPnpmState(profileDir)
          r = await pnpmInstall(profileDir, undefined, [installSpec], 'add')
        }
    ...
    return {
      ...
      profiles: touched,
      restartRequired: true,
      note: '已通过 npm 安装 ' + pkgName + (npmInfo.available && npmInfo.version ? '（' + npmInfo.version + '）' : '') + '（bundles 已注册）。重启 DSH 后生效。',
    }
  }
  ...
  async function runInstallRpc(fullName, repoUrl, kindArg, pkgPathArg, signal) {
    ...
    // 已安装检测：单包仓库已装 → 拒绝。多子包插件集合（共享同一 clone 目录）只装了
    // 部分子包时，允许继续安装“另一个”子包——真正的同包去重留给 clone 后按包名判断。
    if (isInstalled({ fullName })) {
      const installableSubs = getStagedSubPackages(fullName).filter((s) => s && s.installable)
      const subDirCandidate = (pkgPath || 'package.json').split('/').filter(Boolean).slice(0, -1).join('/')
      const isCollectionInstall = installableSubs.length > 1 && !!subDirCandidate
      if (!isCollectionInstall) {
        return { ok: false, error: '该插件已安装（' + fullName + '），如需重装请先卸载' }
      }
    }
    ...
    // 插件集合：多个子包共享同一个 clone 目录。仅当该目录未被任何已安装包引用
    // （孤儿残留）时删除重 clone；被引用时原地更新，以保留其它已安装子包的 link。
    const shared = repoDirReferenced(target)
    const cleanup = () => { if (shared) return; try { rmSync(target, { recursive: true, force: true }) } catch { /* ignore */ } }

    // 已存在目录：未被引用（孤儿残留）→ 删除重 clone；被引用（插件集合的其它子包已
    // 装在此目录下）→ 原地 fetch + reset 更新，保留现有子包链接与 .git。
    ...
    try {
      if (existsSync(target)) {
        if (shared) {
          cloneRes = await gitPull(target, signal)
        } else {
    ...
    // 同包去重（含插件集合：不同子包包名不同，可共存；同一子包重复装则拒绝）。
    // 命中时不调用 cleanup()——插件集合共享目录，删除会破坏其它已安装子包。
    if (scanInstalled().paths.some((x) => x.name === pkgName)) {
      return { ok: false, error: '该插件已安装（' + pkgName + '），如需重装请先卸载' }
    }
  ...
  async function verifyBundleResolvable(realPath, pj) {
    const main = (typeof pj.main === 'string' && pj.main) ? pj.main : null
    const ex = pj.exports && typeof pj.exports === 'object' ? pj.exports : null
    const hostEntry = main && existsSync(join(realPath, main))
      ? join(realPath, main)
      : (() => {
        if (!ex) return null
    ...
    const clientEntry = ex ? (typeof ex['./client'] === 'string' ? ex['./client'] : (ex['./client'] && typeof ex['./client'] === 'object' ? ex['./client'].default : null)) : null
    // 纯客户端插件（只有 ./client 导出）没有宿主端入口，无需做 Node 裸依赖解析。
    if (!hostEntry) {
      if (clientEntry && existsSync(join(realPath, clientEntry))) return { ok: true }
      return { ok: false, error: '无法确定可解析的入口文件（main/exports 均缺失或不存在）' }
    }
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        } else if (path === '/manage/update') {
          result = await manageUpdateRpc(args.name, undefined)
  ...
  const toolDefs = [
    ...
    {
      name: 'market_install',
      description: '获取 DSH 插件源码（cordis.yml / dsh.config / 插件入口文件等清单文件内容）为安装做准备，并识别安装形态（install.kind：dynamic=可 define/run；bundle=可由市场 GUI 一键安装或 dsh plugin add（含 cordis.patch.yml 皮肤类子目录）；list=收录类仓库）。返回 ok、files（含 path 和 content）、install、treeSummary。拿到源码后按形态执行安装：dynamic → cordis_define + cordis_run；bundle → 提示用户使用市场 GUI 一键安装或 dsh plugin add。若 GUI 已点击过安装按钮，源码已在缓存中会立即返回（staged: true）。',
      parameters: {
